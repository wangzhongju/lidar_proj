...
if [ "$(docker ps -aq -f name=$container_name)" ]; then
    ...
    docker run -it -d --rm --name $container_name --net=host -e NVIDIA_VISIBLE_DEVICES=all -e NVIDIA_DRIVER_CAPABILITIES=all --gpus all -e DISPLAY=$DISPLAY -v /tmp:/tmp -v /dev/aks/:/dev/aks/ --privileged -v $work_space:/home/sti/RS_sense_v3 rs_sense_cuda12.0:v1.0.0 
fi
